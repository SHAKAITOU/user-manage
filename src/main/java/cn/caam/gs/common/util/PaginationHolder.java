...
/**
 * A simple state holder for handling lists of objects,
...
public class PaginationHolder {
    ...
    /**
     * Create a new holder instance with the given total rows
     * number and the current page index.
    ...
    /**
     * Create a new holder instance with the given total rows
     * number and the current page index and rows count number and
     * max linked pages count number.
     * @param totalRows Total rows number.
     * @param page The current page index.
     ...
     */
    public PaginationHolder(int totalRows, int page,
            int pageSize, int maxLinkedPages) {
        this.totalRows = totalRows;
        this.page = page;
    ...
    private void initPaginationParameters() {
        float nrOfPages = (float) totalRows / pageSize;
        pageCount = (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages);
